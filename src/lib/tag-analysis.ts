  ...
  try {
    ...
    try {
      ...
      const prompt = `
        ...
        クリエイターを直接肯定しつつ、今後の成長や可能性にも言及してください。

        - 各summaryは最大3行、120文字以内で簡潔かつ肯定的・成長志向でまとめてください。
        - summaryはポジティブな表現・具体的な褒め言葉を必ず含めてください。
        - 総合的な考察には「今後の成長や期待」も1文含めてください。
        - 出力例：
...
async function fallbackSaveTagAnalytics(userId: string, tagAnalysis: TagAnalysisResult): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    const tagClusters = {
      clusters: (tagAnalysis.clusters ?? []) as { name: string; tags: string[] }[],
    };
