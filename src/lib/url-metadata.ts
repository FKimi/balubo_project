...
export async function extractUrlMetadata(url: string): Promise<UrlMetadata> {
  try {
    ...
    // descriptionやtitleから簡易的にタグ候補を抽出（単語分割・重複排除・5件まで）
    const textForTags = `${data.title || ''} ${data.description || ''}`;
    let tags: string[] = [];
    if (textForTags) {
      tags = textForTags
        .replace(/[#,.!?:;[]()"'“”‘’]/g, ' ')
    ...
    if (data['article:published_time']) {
      publishedDate = data['article:published_time'];
    } else if (data['og:published_time']) {
      publishedDate = data['og:published_time'];
    } else if (data['date']) {
