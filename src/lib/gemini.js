...
`;
            // デバッグ用リクエスト内容出力
            console.log('[AI分析API] リクエスト内容:', {
                apiKey: GEMINI_API_KEY ? '***' : '(空)',
                prompt,
                content,
                model: 'gemini-1.5-flash',
                generationConfig,
            ...
            // レスポンス内容を生で出力
            console.log('[AI分析API] 生レスポンス内容:', text);
            // JSON部分を抽出
            ...
            // APIエラー時、詳細ログ出力
            console.error('[AI分析API] API呼び出しエラー:', apiError);
            if (apiError && apiError.response) {
                try {
                    const errorBody = await apiError.response.text?.() || await apiError.response.json?.();
                    console.error('[AI分析API] エラーレスポンスボディ:', errorBody);
                } catch (parseErr) {
                    console.error('[AI分析API] エラーレスポンスパース失敗:', parseErr);
                }
            }
            // エラーの種類に応じたメッセージ
        ...
        console.error('[AI分析API] コンテンツ分析エラー:', error);
        return getFallbackAnalysis();
    }
}
// フォールバック分析を返す関数
