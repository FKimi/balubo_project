 ...
 * @returns 分析結果
 */
async function analyzeUserData(works) {
    try {
        // 全作品からタグを抽出
        const allTags = [];
        works.forEach(work => {
...
// 専門性を分析する関数
function analyzeExpertise(works, tags, categoryCounts) {
    // タグを専門カテゴリに分類
...
// 興味・関心を分析する関数
function analyzeInterests(works, tags, categoryCounts) {
    // タグを興味カテゴリに分類
...
// タグを専門カテゴリに分類する関数
function categorizeTagsByExpertise(tags) {
    // タグをカテゴリにマッピング
...
// タグを興味カテゴリに分類する関数
function categorizeTagsByInterest(tags) {
    // タグをカテゴリにマッピング
...
// 独自性を分析する関数
function analyzeUniqueness(works, tags) {
    // タグと作品内容から独自性を特定
...
// 独自性の特徴を特定する関数
function identifyUniqueTraits(works, tags) {
    // タグの組み合わせから独自性を特定
    const uniqueTraits = [];
    // タグの組み合わせを分析
    const tagPairs = {};
    ...
    works.forEach(work => {
        if (typeof work.description === 'string') {
            commonPhrases.forEach(phrase => {
                if (work.description && work.description.includes(phrase)) {
...
// 才能の要約を生成する関数
function generateTalentSummary(expertise) {
    // 作品数に基づいて経験レベルを判断
...
// 専門分野を生成する関数
function generateSpecialties(tagCounts) {
    // タグの出現回数でソート
...
// デザインスタイルを生成する関数
function generateDesignStyles(works) {
    // デザイン関連のタグを抽出
    const designTags = works.flatMap(work => work.tags || []).filter(tag => ["モダン", "ミニマル", "クリエイティブ", "シンプル", "エレガント", "ポップ", "カラフル"].includes(tag));
